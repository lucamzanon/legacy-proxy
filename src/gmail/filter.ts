...
const SYSTEM: Record<string, string> = { INBOX: "in:inbox", SENT: "in:sent", DRAFT: "in:drafts", SPAM: "in:spam", TRASH: "in:trash", UNREAD: "is:unread", STARRED: "is:starred", IMPORTANT: "is:important", CHAT: "is:chat" };
const keyword = (value: unknown) => {
  ...
  if (typeof value !== "string" || (!value || Buffer.byteLength(value) > 255 || /[\x00-\x20\x7f]/.test(value))) throw invalidArguments("Invalid keyword");
...
export function gmailFilter(filter: unknown, labels: GmailLabel[], depth = 0): string {
  ...
  if (depth > 10 || typeof filter !== "object" || Array.isArray(filter)) throw invalidArguments("Invalid filter");
  ...
  if (f.operator !== undefined) {
    if (!["AND", "OR", "NOT"].includes(String(f.operator)) || !Array.isArray(f.conditions) || !f.conditions.length || Object.keys(f).some((k) => !["operator", "conditions"].includes(k))) throw invalidArguments("Invalid filter operator");
  ...
  for (const [key, value] of Object.entries(f)) {
    ...
    else if (key === "hasAttachment" && typeof value === "boolean") clauses.push(value ? "has:attachment" : "-has:attachment");
    else if (key === "after" || key === "before") {
      if (typeof value !== "string" || !Number.isFinite(Date.parse(value))) throw invalidArguments("Invalid date filter");
      clauses.push(`${key}:${Math.floor(Date.parse(value) / 1000)}`);
    } else if (key === "minSize" || key === "maxSize") {
      if (!Number.isSafeInteger(value) || (value as number) < 0) throw invalidArguments("Invalid size filter");
      clauses.push(`${key === "minSize" ? "larger" : "smaller"}:${key === "minSize" ? Math.max(0, (value as number) - 1) : value}`);
