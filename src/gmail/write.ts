...
const invalid = (description: string): never => { throw new JmapError("invalidProperties", description); };
const KEYWORDS: Record<string, {label:string; inverse?:boolean}> = {
  $seen: {label:"UNREAD",inverse:true}, $flagged: {label:"STARRED"}, $important: {label:"IMPORTANT"},
};
export const writableLabel = (label: GmailLabel): boolean => label.type === "user" || ["INBOX","SPAM","TRASH"].includes(label.id);
...
export function emailPatch(message: GmailMessage, patch: unknown, labels: GmailLabel[]): {addLabelIds:string[];removeLabelIds:string[]} {
  const p=object(patch); const current=new Set(message.labelIds??[]);const desired=new Map<string,boolean>();
  const set=(label:string,value:boolean)=>{
    if(desired.has(label)&&desired.get(label)!==value)invalid("Conflicting mailbox and keyword changes");
    desired.set(label,value);
  };
  const mailbox=(id:string,value:boolean,explicit=true)=>{
    if(id===ALL_MAIL){if(!value)invalid("All mail cannot be removed");return;}
  ...
  const keyword=(key:string,value:boolean)=>{
    ...
    if(mapped){set(mapped.label,mapped.inverse?!value:value);return;}
  ...
  for(const [key,value] of Object.entries(p)){
    if(key==="mailboxIds"){
      ...
      const ids=object(value);if(!Object.keys(ids).length)invalid("A message must remain in a mailbox");
      for(const [id,v] of Object.entries(ids)){if(v!==true)invalid("Mailbox membership must be true");mailbox(id,true);}
      ...
      for(const l of labels)if(writableLabel(l)&&!("l_"+l.id in ids)&&(!archive||l.type!=="user"))set(l.id,false);
    }else if(key==="keywords"){
      if(Object.keys(p).some(k=>k.startsWith("keywords/")))invalid("Overlapping keyword patch paths");
      const keywords=object(value);
      for(const [k,v]of Object.entries(keywords)){if(v!==true)invalid("Keyword value must be true");keyword(k,true);}
      ...
      if(parent==="mailboxIds")mailbox(id,value===true);else keyword(id,value===true);
    }else invalid("Only mailboxIds and keywords can be updated");
  }
  const addLabelIds:string[]=[],removeLabelIds:string[]=[];
  for(const [id,value]of desired)if(current.has(id)!==value)(value?addLabelIds:removeLabelIds).push(id);
...
export function labelInput(input: unknown, create: boolean): {name:string} {
  ...
  for(const [key,value]of Object.entries(p)){
    ...
    if(create&&((key==="parentId"||key==="role")&&value===null||key==="isSubscribed"&&value===true||key==="sortOrder"&&value===0))continue;
    invalid("Only flat label names are supported");
  }
  if(typeof p.name!=="string"||!p.name.trim()||Buffer.byteLength(p.name)>225||/[\x00-\x1f]/.test(p.name))invalid("Invalid label name");
